      ...
      <div className="p-5 md:p-6 space-y-5">
        ...
        {(((effective as any)?.persona?.length) || ((effective as any)?.financial_context)) ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <aside className="md:col-span-1 space-y-4">
              <div className="px-1">
                <div className="flex items-center gap-2 mb-1">
                  <div className="h-6 w-6 rounded-full bg-themeGray/40 flex items-center justify-center ring-1 ring-white/10">
                    <UserRound className="h-3.5 w-3.5 text-[#b9a9ff]" />
                  </div>
                  <h3 className="text-sm font-semibold text-white">Scenario Persona</h3>
                </div>
                <p className="text-xs text-themeTextGray">A fictional character that grounds this example in a real-life story.</p>
              </div>
              {(effective as any)?.persona?.map((p: any, i: number) => (
                <div key={i} className="rounded-xl border border-themeGray/60 bg-gradient-to-b from-[#171a22] to-[#141821] overflow-hidden shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
                  <div className="p-4 flex items-start gap-3">
                    <div className="h-10 w-10 rounded-full bg-themeGray/40 flex items-center justify-center ring-1 ring-white/10">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        {p?.occupation ? (
                          <span className="px-1.5 py-0.5 rounded border border-themeGray/60 bg-white/5 text-[11px] text-themeTextGray">{p.occupation}</span>
                        ) : null}
                        {typeof p?.age === "number" && p.age > 0 ? (
                          <span className="px-1.5 py-0.5 rounded border border-themeGray/60 bg-white/5 text-[11px] text-themeTextGray">Age {p.age}</span>
                        ) : null}
                      </div>
                      {p?.financial_goal ? (
                        <div className="mt-3 rounded-md border border-themeGray/60 bg-[#12151b] p-3 text-themeTextWhite">
                          <div className="flex items-center gap-2 mb-1">
                            <Target className="h-3.5 w-3.5 text-[#b9a9ff]" />
                            <span className="text-[11px] uppercase tracking-wide text-themeTextGray">Financial goal</span>
                          </div>
                          <p className="text-sm leading-relaxed text-white">{p.financial_goal}</p>
                        </div>
                      ) : null}
                    </div>
                  </div>
            ...
            <section className="md:col-span-2 rounded-xl border border-themeGray/60 bg-[#161a20] p-4 shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
              <div className="mb-1 flex items-center gap-2">
                <ClipboardList className="h-4 w-4 text-[#b9a9ff]" />
                <h3 className="text-sm font-semibold text-white">Financial Context</h3>
              </div>
              <p className="text-xs text-themeTextGray mb-3">Key constraints and preferences that shape choices in this scenario.</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {(effective as any).financial_context?.time_horizon ? (
                  <div className="rounded-lg border border-themeGray/60 bg-[#12151b] p-3 text-themeTextWhite flex items-start gap-2 min-h-[90px] hover:bg-[#141821] transition-colors">
                    <Clock className="h-4 w-4 text-[#b9a9ff] mt-0.5" />
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-themeTextGray">Time horizon</p>
                      <p className="text-sm text-white">{(effective as any).financial_context.time_horizon}</p>
                    </div>
                  </div>
                ) : null}
                {(effective as any).financial_context?.available_amount ? (
                  <div className="rounded-lg border border-themeGray/60 bg-[#12151b] p-3 text-themeTextWhite flex items-start gap-2 min-h-[90px] hover:bg-[#141821] transition-colors">
                    ...
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-themeTextGray">Available amount</p>
                      <p className="text-sm text-white">{(effective as any).financial_context.available_amount}</p>
                    </div>
                  </div>
                ) : null}
                {(effective as any).financial_context?.risk_tolerance ? (
                  <div className="rounded-lg border border-themeGray/60 bg-[#12151b] p-3 text-themeTextWhite flex items-start gap-2 min-h-[90px] hover:bg-[#141821] transition-colors">
                    ...
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-themeTextGray">Risk tolerance</p>
                      <p className="text-sm text-white">{(effective as any).financial_context.risk_tolerance}</p>
                    </div>
                  </div>
                ) : null}
                {(effective as any).financial_context?.current_situation ? (
                  <div className="rounded-lg border border-themeGray/60 bg-[#12151b] p-3 text-themeTextWhite flex items-start gap-2 min-h-[90px] hover:bg-[#141821] transition-colors">
                    ...
                    <div>
                      <p className="text-[11px] uppercase tracking-wide text-themeTextGray">Current situation</p>
                      <p className="text-sm text-white">{(effective as any).financial_context.current_situation}</p>
                    </div>
                  </div>
                ) : null}
              </div>
