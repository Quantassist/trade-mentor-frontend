...
export const useActiveGroupSubscription = (groupid: string) => {
  const { data } = useQuery({
    queryKey: ["active-group-subscription"],
...
export const useGroupSubscription = (groupid: string) => {
  ...
  const { mutate, isPending, variables } = useMutation({
    mutationFn: async (data: z.infer<typeof CreateGroupSubscriptionSchema>) =>
...
export const useAllSubscriptions = (groupid: string) => {
  const { data } = useQuery({
    queryKey: ["group-subscriptions"],
    queryFn: () => onGetGroupSubscriptions(groupid), //TODO: Verify the correctness
  })
