...
export const CourseCreate = ({ groupid }: CourseCreateProps) => {
  const {
    onCreateCourse,
    register,
    errors,
  ...
  if (data?.groupOwner) {
    return (
      <>
        ...
        {isPending && variables && (
          <Link href={`/group/${groupid}/courses/${variables.id}`}>
            <Card className="bg-transparent border-themeGray h-full rounded-xl overflow-hidden">
              ...
              <div className="h-2/6 flex flex-col justify-center pl-5">
                <h2 className="text-lg text-themeTextGray font-semibold">
                  {variables.name}
                </h2>
                <p>{truncateString(variables.description)}</p>
              </div>
            </Card>
          </Link>
        )}
